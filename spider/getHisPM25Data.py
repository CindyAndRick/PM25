...
if __name__ == '__main__':
    ...
    while(1):
        url = baseUrl + str(y) + (('0' + str(m)) if m < 10 else str(m)) + (('0' + str(d)) if d < 10 else str(d)) + '.csv'
        r = requests.get(url, headers=headers)
        '''
        with open(file='./spider/data{}.csv'.format(str(y)+str(m)+str(d)), mode='w+', encoding='utf-8') as f:
            f.write(r.text)
            f.close()
        '''
